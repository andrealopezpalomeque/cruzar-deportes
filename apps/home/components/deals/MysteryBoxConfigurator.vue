  ...
  <Card class="p-8">
    <form @submit.prevent="handleSubmit" class="space-y-8">
        ...
        >
          <div class="flex items-center">
            ...
            <span
              ...
              :class="currentStep >= index + 1 ? 'text-ink' : 'text-ink-muted'"
            >
              {{ step }}
            </span>
          </div>
          <div
            ...
            :class="currentStep > index + 1 ? 'bg-ink' : 'bg-surface-muted'"
          ></div>
        </div>
      </div>
      ...
      <div v-show="currentStep === 1" class="space-y-4">
        <div>
          <label class="block text-lg font-medium text-ink mb-4">
      ...
      <div v-show="currentStep === 3" class="space-y-4">
        <div>
          ...
          <p class="text-sm text-ink-muted mt-2">Seleccioná hasta 5 equipos que no querés recibir</p>
        </div>
      </div>
      ...
      <div v-show="currentStep === 4" class="space-y-4">
        <div>
          ...
          <p class="text-sm text-ink-muted mt-2">Elegí qué tipo de camisetas preferís recibir</p>
        </div>
      </div>
      ...
      <div v-show="currentStep === 5" class="space-y-6">
        ...
        <div>
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-ink-muted mb-1">Nombre completo *</label>
              <input
                type="text"
                v-model="formData.customer.name"
                class="w-full px-4 py-2 border border-surface-muted rounded-sm focus:outline-none focus:ring-2 focus:ring-brand-orange-500 focus:border-transparent"
            ...
            <div>
              <label class="block text-sm font-medium text-ink-muted mb-1">Dirección de envío *</label>
              <textarea
            ...
            <div>
              <label class="block text-sm font-medium text-ink-muted mb-2">Método de pago *</label>
              <div class="space-y-2">
                <label
                  v-for="method in paymentMethods"
                  :key="method.value"
                  class="flex items-center p-3 border rounded-sm cursor-pointer transition-colors"
                  :class="formData.customer.paymentMethod === method.value ? 'border-ink bg-surface-warm' : 'border-surface-muted hover:border-surface-muted'"
                >
                  <input
                    type="radio"
                    :value="method.value"
                  ...
                  <div
                    :class="[
                      ...
                      formData.customer.paymentMethod === method.value ? 'border-ink' : 'border-surface-muted'
                    ]"
                  >
                    <div
                      v-if="formData.customer.paymentMethod === method.value"
                      class="w-2 h-2 min-w-[8px] min-h-[8px] rounded-full bg-ink"
                    ></div>
                  </div>
        ...
        <!-- Summary -->
        <div class="p-6 bg-surface-warm rounded-sm">
          <h3 class="font-medium text-ink mb-4">Resumen de tu Configuración</h3>
          <div class="space-y-2 text-sm">
            <div class="flex justify-between">
              <span class="text-ink-muted">Tipo de Caja:</span>
              <span class="font-medium">{{ boxLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">Cantidad:</span>
              <span class="font-medium">{{ jerseyCount }} camiseta(s)</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">Talles:</span>
              <span class="font-medium">{{ formattedSizes || '-' }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">Equipos excluidos:</span>
              <span class="font-medium">{{ formData.excludedTeams.length || 'Ninguno' }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">Preferencia:</span>
              <span class="font-medium">{{ getEraLabel(formData.eraPreference) }}</span>
            </div>
            <div class="flex justify-between items-center pt-4 border-t border-surface-muted">
      ...
      <!-- Navigation Buttons -->
      <div class="flex flex-col sm:flex-row justify-between gap-3 pt-6 border-t border-surface-muted">
        <Button
          v-if="currentStep > 1"
          type="button"
          variant="outline"
